...
public class CustomCommandRule : Rule {
    ...
    public override func invokeWithModifiedFiles(files: [ProjectFile], result: LROperationResult, completionHandler: dispatch_block_t) {
        ...
        let command = self.command!.substituteValues(info)
        let shell = "/bin/bash"
        ...
        NSLog("Executing project rule command: %@", (([shell] + shArgs) as NSArray).p2_quotedArgumentStringUsingBourneQuotingStyle());
