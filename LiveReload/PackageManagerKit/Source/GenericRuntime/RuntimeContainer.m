...
- (void)updateInstancesWithData:(NSArray *)instancesData {
    NSDictionary *keyedOldInstances = [_instances at_keyedElementsIndexedByValueOfKeyPath:@"identifier"];
    NSDictionary *keyedNewData = [instancesData at_keyedElementsIndexedByValueOfKeyPath:@"identifier"];
    [keyedNewData at_enumeratePairsMatchingObjectsInDictionary:keyedOldInstances usingBlock:^(NSDictionary *_Nullable newData, RuntimeInstance *_Nullable instance) {
        if (instance && newData) {
            // no update
        } else if (newData) {
            instance = [self newRuntimeInstanceWithData:newData];
