...
public class LRContextAction: NSObject {
    ...
    public func newInstance(#memento: [String: AnyObject]) -> Rule {
    ...
    private func _computeAvailableVersions() -> [LRActionVersion] {
        ...
        let packageSets = flatten(action.packageConfigurations.map { self.resolutionContext.packageSetsMatchingPackageReferences($0.packageReferences) as! [LRPackageSet] })

        let versions = mapIf(packageSets) { packageSet -> LRActionVersion? in
        ...
        let sortedVersions = versions.sorted { (a: LRActionVersion, b: LRActionVersion) -> Bool in
