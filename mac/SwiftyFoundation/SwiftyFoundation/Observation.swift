...
public struct Observation {
    ...
    public mutating func on(name: String, object: AnyObject? = nil, block: (NSNotification) -> Void) {
        observations.append(NotificationObservationProxy(name: name, object: object, block))
    }

    public mutating func on <T: AnyObject> (name: String, object: AnyObject? = nil, _ observer: T,  block: (T) -> (NSNotification) -> Void) {
    ...
    public mutating func on <T: AnyObject> (name: String, object: AnyObject? = nil, _ observer: T,  block: (T) -> () -> Void) {
        weak var weakObserver: T? = observer
        on(name, object: object, { notification in
            if let observer = weakObserver {
                block(observer)()
