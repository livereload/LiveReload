(function() {
  ...
  URLOverrideCoordinator = (function() {
    ...
    URLOverrideCoordinator.prototype.handleHttpRequest = function(url, callback) {
      var baseUrl, content, errCode, fileType, localPath, _ref, _ref1;
      ...
      await(this.Path.exists(localPath, defer(exists)));
      if (!exists) {
      ...
      if (fileType.isCSS && baseUrl) {
        await(this.fs.readFile(localPath, 'utf8', defer(err, content)));
        if (err) {
          return callback(err);
        }
