...
class FSTree extends EventEmitter
  ...
  _addFile: (relpath, stats) ->
    stats = { size: stats.size, mode: stats.mode, mtime: stats.mtime, dev: stats.dev, ino: stats.ino, isFile: stats.isFile() }
  ...
  _walk: (path, relpath, autocb) ->
    ...
    if stats.isFile()
      # debug "@filter.matches('#{relpath}', no) == #{@filter.matches(relpath, no) && 'yes' || 'no'}"
      if @filter.matches(relpath, no)
        @_addFile relpath, stats
    else if stats.isDirectory()
      # debug "@filter.excludes('#{relpath}', yes) == #{@filter.excludes(relpath, yes) && 'yes' || 'no'}"
