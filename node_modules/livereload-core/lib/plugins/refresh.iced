...
class RefreshStep
  ...
  schedule: (change) ->
    @queue.add { project: @project.id, action: 'refresh', reloadRequests: change.reloadRequests.slice(0) }
  ...
  _perform: (request, done) ->
    ...
    return done(null) if request.reloadRequests.length == 0
    ...
    for reloadRequest in request.reloadRequests
      command =
        command:        'reload'
        path:           reloadRequest.path
        originalPath:   reloadRequest.originalPath or ''
