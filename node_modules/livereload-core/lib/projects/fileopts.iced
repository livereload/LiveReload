...
class FileOptions
  ...
  schema:
    ...
    relpath:                  { reactive: no }
    fullPath:                 { reactive: no }
    destDir:                  { reactive: no }
    fullDestDir:              { reactive: no }
    destName:                 { reactive: no }
    destRelPath:              { reactive: no }
    isImported:               { reactive: no }
  ...
  get_fullPath: -> Path.join(@project.fullPath, @path)

  get_destDir:     -> @outputDir
  set_destDir: (v) -> @outputDir = v

  get_fullDestDir:     -> Path.join(@project.fullPath, @destDir)
  set_fullDestDir: (v) -> @destDir = Path.relative(@project.fullPath, v)

  get_destName: -> @outputNameMask and @outputNameForMask(@outputNameMask) or "<none>"
  get_destRelPath: -> Path.join(@outputDir, (@outputNameMask and @outputNameForMask(@outputNameMask) or "<none>"))

  get_isImported: -> @project.imports.hasIncomingEdges(@path)
