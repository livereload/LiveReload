...
class Project
  ...
  initialize: ({ @session, @vfs, @path }) ->
    ...
    @watcher = fsmonitor.watch(@fullPath, @session.monitoringFilesFilter, mode: 'manual')
    ...
    @session.after =>
      debug "Analysis complete for #{@fullPath}"
      @broadcastChange 'fileOptionsByPath'
      @emit 'complete'
  ...
  # automatically_update_ruleSet_availableActions: ->
  ...
  compute_availableCompilers: ->
    @session.pluginManager?.allCompilers or []


  compute_compilableFilesFilter: ->
  ...
  matchesPath: (root, path) ->
    ...
    @vfs.isSubpath(@fullPath, fullPath)
  ...
  handleChange: (vfs, root, paths) ->
    ...
    change = { paths, reloadRequests: ({ path: path, originalPath: null } for path in paths) }
    ...
    @session.queue.checkpoint =>
      ...
      change.reloadRequests = ({ path: path, originalPath: null } for path in pathsToProcess)
