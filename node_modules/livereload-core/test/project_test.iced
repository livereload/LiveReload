...
describe "Project", ->
  ...
  it "should be reactive", (done) ->
    ...
    class Monitor
      automatically_readOptions: ->
        @disableLiveRefresh = project.disableLiveRefresh
        @compilationEnabled = project.compilationEnabled

    await universe.performAndWait (-> universe.create(Monitor)), defer(err, monitor)
    throw err if err
    # sanity check
    strictEqual monitor.disableLiveRefresh, no
    strictEqual monitor.compilationEnabled, no

    await
      project.once 'complete', defer()
      project.setMemento { disableLiveRefresh: 1, compilationEnabled: 1 }

    strictEqual monitor.disableLiveRefresh, yes
    strictEqual monitor.compilationEnabled, yes
