...
class REntitySchema
  ...
  addClass: (klass) ->
    throw "#{this}: cannot add a mixin to a frozen entity" if @_frozen
  ...
  _handleExtendsKey: (value, errorContext) ->
    ...
    for item in value
      unless typeof item is 'string'
        throw new Error("Invalid item type '#{typeof item}', expected a kind name (string) in #{errorContext.join(', ')}")
      @extend(item)
...
class RAttributeSchema
  constructor: (@entitySchema, @name, @options) ->
    ...
    if @options.collection
      ...
      if @itemType.isEntityTypeDescriptor
        @itemEntitySchema = @universe.resolveEntityKind(@itemType.kind)
      else
