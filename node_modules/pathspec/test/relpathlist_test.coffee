...
o '!CVS', ['!CVS.txt', 'CVS', 'CVS/xxx.txt', '!bar', '!bar/CVS.txt', 'bar/CVS/xxx.txt']
o '!.git', ['!.git.txt', '.git', '.git/xxx.txt', '!bar', '!bar/.git.txt', 'bar/.git/xxx.txt']

describe "RelPathList (excludes)", ->
      ...
      describe "like '#{specStr}'", ->
        ...
        for example in examples
          do (example) ->
            if example[0] == '!'
              example = example.substr(1)
              it "should not exclude '#{example}'", ->
                assert.ok !spec.excludes example
            else
              it "should exclude '#{example}'", ->
                assert.ok spec.excludes example
