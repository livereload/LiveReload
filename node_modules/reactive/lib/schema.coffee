...
class RModelSchema
  ...
  constructor: (@universe, originalModelClass) ->
    @modelClass = @_createSingletonClass(RModel, originalModelClass.name)
    @mixinClasses = []
  ...
  _createSingletonClass: (modelClass, newClassName) ->
    ## This would be a sane way to do this, if Function.name wasn't unassignable
    # singletonClass = (args...) ->
    #   modelClass.apply(this, args)
    # singletonClass.name = newClassName

    # so let's do it the insane way
    global.REACTIVE_CLASS_CREATION_HACK = modelClass
    singletonClass = eval("(function(modelClass) { return function #{newClassName}() { modelClass.apply(this, arguments); }; })(global.REACTIVE_CLASS_CREATION_HACK);")
  ...
  _extendModel: (mixinClass) ->
    ...
    for own k, v of mixinClass
      # workaround for how __extends behaves in CoffeeScript; can be removed when model classes stop deriving from R.Model
      continue if RModel.hasOwnProperty(k)
    ...
    for k, v of mixinClass.prototype when !(k in ['schema', 'constructor', 'initialize'])
      # workaround for how __extends behaves in CoffeeScript; can be removed when model classes stop deriving from R.Model
      continue if RModel.prototype.hasOwnProperty(k)
